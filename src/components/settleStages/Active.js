...
class Active extends Component {
					...
					.then(() => {
						...
						if (
							this.props.participants.find(
								(e) => e.user_id === this.props.user.user_id
							)
						) {
							this.setState({
								loaded: true,
	...
	removeSuggestion = (e) => {
		const { socket } = this.props;
		axios
			.put(`/api/settle/${this.props.id}/remove`, {
				suggestion: e,
				...
				axios
					...
					.then((response) => {
						...
						if (response.data.length === 1) {
							axios
								.put(`/api/settle/${this.props.id}/stage`, {
									status: "completed",
								})
								.then(() => {
		...
		const list = this.state.suggestions.map((e, i) => {
			return (
				<div className="listitem" key={i}>
					...
					<button
						className={
							this.state.activeuser === this.props.user.user_id
								? "removesuggestion"
								: "hide"
						}
						onClick={() => this.removeSuggestion(e)}
					>
						<img
